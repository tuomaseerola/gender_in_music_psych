...
#dim(df)
#_df2 <- df %>% filter(!is.na(sample_size) | !sample_id=='sample0')
#dim(df2)
#head(df[,24:28],10)
#head(df2[,24:28],10)
...
#### 10. Gender balance and WEOG --------------------
cat("10. Gender balance and WEOG\n")
...
WEOG_countries<-c("Andorra","Australia","Austria","Belgium","Canada","Denmark","Finland","France","Germany","Greece","Iceland","Ireland","Israel","Italy","Liechtenstein","Luxembourg","Malta","Monaco","Netherlands","New Zealand","Norway","Portugal","San Marino","Spain","Sweden","Switzerland","Turkey","UK","US","Vatican City")
...
index[is.na(index)]<-FALSE
d$CountryDataCollected[index]<-d$SamplePrimaryCountryofOrigin_online_inferred[index]# [todo]
table(d$SamplePrimaryCountryofOrigin_online_inferred)
...
SPLIT <- mean(unique(d$USDC),na.rm = T) # calculate this from the unique index, rather than from our data 
SPLIT
...
#### 11. Weighted Age ------------
cat('11. Calculate weighted mean and sd age \n')
...
rm(t,k,l,sample_id,new_row,SAMPLE,pb,USD,WAN,tmp,index,ind,id) # tmp variables

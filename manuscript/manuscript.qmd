...
[TE: is this supporting the idea that women are publishing more access than men? Or is this more about the lack of senior positions, which are probably not going to be achieved with internal service?]**
...
There are certain limitations of the current research. First, the gender of the authors are not part of the database - we infer the gender based on the name using XYZ. Despite this, because the field is relatively small, a check from both authors meant that a certain number of the more ambiguous names (in terms of their gender) could be fixed (see accompanying code). Additionally, the error (confidence) of the gender attribution was so small, that it is unlikely that the remaining names that were not necessarily attributed correctly would significantly change the main conclusions. Another limitation is that while we recognise that gender is a complex and multidimensional social construct, there are limitations to treating gender as a binary category. We used a binary male-female classification consistent with prior meta-science research on gender and authorship [@son2022scientific; @shah2021gender; @rock2021quantifying; @ni2021gendered; @gonzalez2020women; @wais2006] and generally the number of non-binary authors is currently relatively low (0.8% @son2022scientific). Nonetheless, future research should also move towards using more inclusive, non-binary information. 

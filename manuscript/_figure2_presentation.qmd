...
df$authortype[df$author_single == 1] <- 'Other'
...
fig2b <- ggplot(author_N,aes(x=author_N_cropped,y=Odds_ratio,color=Type))+
  ...
  xlab('Number of authors/article')+
  ylab('Female Authorship Odds Ratio')+
  theme_classic(base_size = 15)
#print(fig2)
library(ggdark)
fig2b <- fig2b + dark_theme_bw(base_size = 16) + theme(legend.position="top")
...
S<-s
S$n[S$n > 9]<-10
fig2a <- ggplot(S,aes(x=n))+
  geom_histogram(binwidth=1,fill='gold1',color='black')+
  theme_classic(base_size = 15)+
  labs(x='Number of authors/article',y='Count of articles')+
...
# apply dark theme
fig2a <- fig2a + dark_theme_bw(base_size = 16) + theme(legend.position="top")
...
G<-cowplot::plot_grid(fig2a,fig2b,nrow=1,labels=c('A','B'),label_colour = "white",rel_widths = c(0.6,1))
print(G)
ggsave("figure2_authorship_by_number_of_authors_dark.png",
  G,
  width = 8.1,
  height = 4,
  dpi = 300

...
source('scripts/attribute_gender.R') # Adds gender attributions (precalcutaed with API), R1
source('scripts/name_gender_fixes.R') # Fixes the gender attribution of some names manually
source('scripts/create_keys.R') # Add keys for authors and study+author, R1
...
source('scripts/export_gender_data.R') # save data (remove emails), R1

#### Analyse --------
source('scripts/load_gender_data.R') # Loads the data, R1
source('scripts/summarise_gender.R') # Descriptives, R1
...
source('scripts/citations.R')        # R1 (revised to average citations)
